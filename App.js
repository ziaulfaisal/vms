...
// Current app version — bump this with each new build
    ...
    <View style={stylesUpdate.container}>
      ...
      <View style={stylesUpdate.content}>
        ...
        <Text style={stylesUpdate.subtitle}>
          {updateInfo?.message ||
            "You're on an older app version. We require you to update to the latest version for an improved experience."}
        </Text>

        {updateInfo?.version && (
          <View style={stylesUpdate.versionBadge}>
            <Text style={stylesUpdate.versionText}>Latest Version: v{updateInfo.version}</Text>
          </View>
        )}

        <TouchableOpacity style={stylesUpdate.updateBtn} onPress={handleUpdate} activeOpacity={0.85}>
          <Icon name="download" type="material-community" size={20} color="#fff" style={{ marginRight: 8 }} />
          <Text style={stylesUpdate.updateBtnText}>Update Application</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={stylesUpdate.exitBtn}
...
// ============================================
// FORCE UPDATE CHECK (from server appupdate.json)
// ============================================
async function checkForceUpdate() {
  try {
    const res = await fetch(APP_UPDATE_URL, { cache: 'no-store' });
    if (!res.ok) return null;
    const data = await res.json();
    // data shape: { force_update: true/false, version: "2.1.0", download_url: "...", message: "..." }
    if (data.force_update === true) {
      // Compare versions: if server version > current, force update
      const serverVer = (data.version || '0.0.0').split('.').map(Number);
      const currentVer = CURRENT_VERSION.split('.').map(Number);
      for (let i = 0; i < 3; i++) {
        if ((serverVer[i] || 0) > (currentVer[i] || 0)) return data;
        if ((serverVer[i] || 0) < (currentVer[i] || 0)) return null;
      }
    }
    return null;
  } catch (e) {
...
// ============================================
// QR Scanner Modal — uses CameraView (expo-camera v14)
// ============================================
      ...
      <Modal visible={visible} animationType="slide" transparent={false}>
        <View style={stylesScanner.modalContainer}>
          ...
          <Button
            ...
            buttonStyle={{ backgroundColor: '#1a73e8', marginBottom: 12 }}
          />
          <Button
        ...
        >
          <View style={stylesScanner.overlay}>
            ...
            <View style={stylesScanner.scanArea}>
              ...
              <Text style={stylesScanner.scanText}>
                {scanned ? '✅ Scanned!' : 'Align QR code within frame'}
              </Text>

              {scanned && (
                <TouchableOpacity
                  onPress={() => setScanned(false)}
                  style={[stylesScanner.manualEntryBtn, { backgroundColor: 'rgba(76,175,80,0.85)' }]}
                >
                  <Text style={stylesScanner.manualEntryText}>Tap to scan again</Text>
                </TouchableOpacity>
              )}

              <TouchableOpacity onPress={() => setShowManual(true)} style={stylesScanner.manualEntryBtn}>
                <Text style={stylesScanner.manualEntryText}>✏️ Enter manually</Text>
              </TouchableOpacity>
            </View>
          </View>
        </CameraView>
...
const stylesScanner = StyleSheet.create({
  ...
  manualEntryText: { color: '#fff', fontSize: 14, fontWeight: '500' },
...
const stylesDash = StyleSheet.create({
  ...
  statCard: {
    width: CARD_WIDTH, marginHorizontal: 6, marginBottom: 10, backgroundColor: '#fff',
    borderRadius: 12, alignItems: 'center', paddingVertical: 16, elevation: 2,
  ...
  menuCard: {
    width: CARD_WIDTH, marginHorizontal: 6, marginBottom: 12, backgroundColor: '#fff',
    borderRadius: 12, alignItems: 'center', paddingVertical: 20, elevation: 2,
    shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 4, shadowOffset: { width: 0, height: 2 },
  },
    ...
    try {
      ...
      if (infoRes.success && infoRes.data) {
        setForm(f => ({
          ...
          vehicle_number: infoRes.data.vehicle_number || '',
          make: infoRes.data.make || '',
          model: infoRes.data.model || '',
          km_reading: infoRes.data.km_reading || '',
          mobile_number: infoRes.data.mobile_number || '',
        }));
      }
        ...
        <View style={stylesForm.modalOverlay}>
          <Card containerStyle={stylesForm.modalCard}>
            ...
            <Text style={stylesForm.modalText}>
              Token No: {duplicateData}{'\n'}Status: Already in premises, not exited yet.
            </Text>
    ...
    try {
      ...
      if (res.success) {
        Alert.alert('Success ✅', `Gate Out recorded at ${res.timestamp}`, [{
          text: 'OK', onPress: () => {
            setScannedData(''); setVehicleInfo(null);
          ...
          <View style={stylesStock.locationGrid}>
            {LOCATIONS.map((loc) => (
              <Chip
                ...
                title={loc}
                type="outline"
                onPress={() => { setLocation(loc); setShowForm(true); }}
    ...
    <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      <View style={stylesReports.tabs}>
        {TABS.map((t) => (
          <TouchableOpacity
            key={t.key}
            style={[stylesReports.tab, tab === t.key && { backgroundColor: t.color, borderRadius: 8, marginHorizontal: 4 }]}
...
export default function App() {
  ...
  useEffect(() => {
    async function init() {
      // 1. Check OTA update first (no rebuild needed, just JS patch)
      await checkForOTAUpdate();

      // 2. Check force update from server
      const updateData = await checkForceUpdate();
      if (updateData) {
  ...
  if (checking) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f8faff' }}>
        <Icon name="car" type="material-community" size={48} color="#1a73e8" />
        <ActivityIndicator size="large" color="#1a73e8" style={{ marginTop: 16 }} />
        <Text style={{ color: '#999', marginTop: 12, fontSize: 13 }}>Checking for updates...</Text>
      </View>
    );
  }
